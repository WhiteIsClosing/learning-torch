...
gnuplot.plot({'Train', torch.range(1, #losses),torch.Tensor(losses),'-'}, {'Test', torch.range(1, #testlosses),torch.Tensor(testlosses),'-'})
gnuplot.plotflush()
...
local groundtruth = test.labels:type('torch.IntTensor')
local prediction = classPredictions:type('torch.IntTensor')
local acc = torch.eq(groundtruth, prediction)
print(acc:sum()/10000)
